...
void do_child(int fd, char *comando, char **args){
    ...
    if(pid == -1){
            ...
            dup2(fd,1);
            close(fd);
            if(execvp(comando, args) == -1) perror("1 - erro a fazer o exec");
            _exit(-1);
    }
    else{
            wait(NULL);
            ...
            for(int i=0;i<n_fdesc;i++) 
                if(fdesc[i].leitura == 1){
                        ...
                        if(pid == -1){
                                ...
                                //printf("fdx %d - %d\n",n,nodes[n_nodes].num);
                                if(n==0){ perror("Termina a leitura do pip do filho");  _exit(-1);}
                                printf("fdx %d - %d -> %s\n",n,nodes[n_nodes].num,resultado);
                                //resultado[n] = '\0';
                                int pipeAux[2];
                                ...
                                for(int j=0;j<n_fdesc;j++){
                                    ...
                                    if(pid == -1){
                                        ...
                                        if(execvp(nodes[n_nodes].command[0], nodes[n_nodes].command) == -1) perror("2 -erro a fazer o exec");
                                        _exit(-1);
                                }
                                else{
                                    close(pipeAux[0]);
            ...
            for(int i=0;i<n_fdesc;i++){
                if(fdesc[i].escrita == 1){
                        ...
                        if(pid == -1){
                                ...
                                close(pipeAux[0]);
                                printf("command0 - %s\n",nodes[n_nodes].command[0]);
                                printf("command1 - %s\n",nodes[n_nodes].command[1]);
                                printf("command2 - %s\n",nodes[n_nodes].command[2]);
                                
                                dup2(fdesc[i].filD,1);
                                close(fdesc[i].filD);

                                if(execvp(nodes[n_nodes].command[0], nodes[n_nodes].command) == -1) perror("3 - erro a fazer o exec");
                                _exit(-1);
                        }
                        else{
                                close(pipeAux[0]);
